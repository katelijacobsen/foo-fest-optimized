...
function LineUpCard({ name, logo, scene, day, start, members, bio, end, logoCredits, open, onClose, onOpen }) {
  ...
  return (
    <>
      <div className=" w-[350px] md:w-[400px] h-[300px] cursor-pointer relative group" onClick={onOpen}>
        <div>
        ...
        <div className="fixed inset-0 z-30 flex justify-center items-start bg-customBlack bg-opacity-50" onClick={onClose}>
          <motion.div style={{ marginTop: `${modalPosition}px` }} variants={modalVisibility} initial="hidden" animate="visible" exit="exit" transition={{ duration: 0.5, ease: "easeInOut" }} className="bg-customBlack_5 w-[98%] md:w-[55%] max-h-[5%] md:max-h-[20%] rounded-sm ">
            ...
            <div className="p-4 grid gap-4">
              <h2 className="font-bold text-2xl md:text-3xl pb-4">{name}</h2>
...
//   // Variabler til modalens animation
//   const modalVisibility = {
//     hidden: { y: "100%", opacity: 0 },
//     visible: { y: "4%", opacity: 1 },
//     exit: { y: "100%", opacity: 0 },
...
//   // Find brugerens scrollposition, når modalet åbnes
//   useEffect(() => {
//     if (open) {
//       const scrollY = window.scrollY || 0;
//       setModalPosition(scrollY); // Gem brugerens position
//     }
//   }, [open]);

//   return (
//     <>
//       {/* Kortet med hover-effekt */}
//       <div className="w-[350px] md:w-[400px] h-[300px] cursor-pointer relative group" onClick={onOpen}>
//         <div className="w-full h-64">
//           {imageUrl ? (
//             <>
//               <Image className="hover:brightness-50 transition ease-in-out duration-75 w-full h-full object-cover" quality={75} src={imageUrl} width={199} height={128} alt={`${name} logo - ${logoCredits}`} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" priority={false} loading="lazy" />
//               {/* Hover overlay med bandnavn */}
...
//       {open && (
//         <div
//           className="fixed inset-0 z-30 flex justify-center items-start bg-customBlack bg-opacity-50"
//           onClick={onClose} // Luk modalet, hvis der klikkes udenfor
//         >
//           <motion.div
//             style={{ marginTop: `${modalPosition}px` }}
//             variants={modalVisibility}
//             initial="hidden"
//             animate="visible"
//             exit="exit"
//             transition={{ duration: 0.5, ease: "easeInOut" }}
//             className="bg-customBlack_5 w-[98%] md:w-[55%] max-h-[90vh] rounded-sm relative"
//             onClick={(e) => e.stopPropagation()} // Forhindrer lukning, når man klikker inde i modalet
//           >
//             <div className="relative">
//               <button className="absolute top-2 right-2 m-2" alt="exit modal" onClick={onClose}>
//                 <RxCross2 className="bg-customBlack_5 text-customOrange h-10 w-10 md:ml-0 md:mb-8 border-solid border-[1px] border-customOrange rounded-full p-2" />
//               </button>
//               <Image className="w-full h-[15rem] md:h-[30rem] object-cover" quality={75} src={imageUrl} width={199} height={128} alt={`${name} logo - ${logoCredits}`} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" priority={false} loading="lazy" />
//             </div>
//             <div className="p-4 grid gap-4">
